<?php
/**
 * @package Internet Quran Radio
 * @version 1.2
 */
/*
 Plugin Name: Internet Quran Radio
 Plugin URI: http://www.mp3quran.net
 Description: Internet Quran Radio plugin more than 120 quran radios.
 Version: 1.2
 Author: Mp3Quran.net
...
$word['title'] = 'Internet Quran Radio';
$word['error'] = 'Error ID!';
...
function mp3quran_content_replace_all_radios($text){
	$text = str_replace( '[Radios]', mp3quran_get_all_radio(), $text );
	return $text;
}

function mp3quran_content_replace($t){
	$text = preg_replace_callback("/R\[([0-9]*?)\]/s", "mp3quran_get_radio", $t);
	return $text;
}
 
add_filter('the_content','mp3quran_content_replace');
add_filter('the_content','mp3quran_content_replace_all_radios');
